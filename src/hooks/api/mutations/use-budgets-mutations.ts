...
type CreateBudgetVariables = {
  householdId: string
  userId?: string | null
} & Omit<CreateBudgetRequest, 'startDate'> & {
...
type UpdateBudgetVariables = {
  id: string
  userId?: string | null
} & Omit<UpdateBudgetRequest, 'startDate'> & {
    startDate?: string | Date
  }
...
export function useCreateBudget(
  callbacks?: MutationCallbacks<CreateBudgetResponse, CreateBudgetVariables>
) {
...
export function useUpdateBudget(
  callbacks?: MutationCallbacks<UpdateBudgetResponse, UpdateBudgetVariables>
) {
  const queryClient = useQueryClient()
      ...
      return mutationFn(
        {
          ...
          body: {
            ...rest,
            startDate: startDate ? toApiDateRequired(startDate) : undefined
          }
        },
        {} as never
      )
...
export function useLinkBudgetCategory(
  callbacks?: MutationCallbacks<unknown, BudgetCategoryVariables>
) {
  const queryClient = useQueryClient()
  ...
  return useMutation<unknown, Error, BudgetCategoryVariables>({
    mutationFn: async (variables: BudgetCategoryVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing linkCategoryToBudget mutation function')
      return mutationFn(
        {
          path: {
...
export function useLinkBudgetAccount(
  callbacks?: MutationCallbacks<unknown, BudgetAccountVariables>
) {
  const queryClient = useQueryClient()
  ...
  return useMutation<unknown, Error, BudgetAccountVariables>({
    mutationFn: async (variables: BudgetAccountVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing linkAccountToBudget mutation function')
      return mutationFn(
        {
          path: {
            budgetId: variables.budgetId,
...
export function useSetDefaultBudget(
  ...
  >({
    mutationFn: async (variables: SetDefaultBudgetVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing setDefaultBudget mutation function')
      return mutationFn(
        {
          body: {
