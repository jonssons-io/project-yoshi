...
type ArchiveCategoryVariables = {
  id: string
  archived: ArchiveCategoryRequest['archived']
  userId?: string | null
}
...
export function useCreateCategory(
  callbacks?: MutationCallbacks<CreateCategoryResponse, CreateCategoryVariables>
) {
  ...
  return useMutation<CreateCategoryResponse, Error, CreateCategoryVariables>({
    mutationFn: async (variables: CreateCategoryVariables) => {
      ...
      if (!mutationFn)
        throw new Error('Missing createCategory mutation function')
      return mutationFn(
        {
          path: {
            householdId
...
export function useUpdateCategory(
  callbacks?: MutationCallbacks<UpdateCategoryResponse, UpdateCategoryVariables>
) {
  const queryClient = useQueryClient()
...
export function useDeleteCategory(
  ...
  return useMutation<DeleteCategoryResponse, Error, DeleteCategoryVariables>({
    mutationFn: async (variables: DeleteCategoryVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing deleteCategory mutation function')
      return mutationFn(
        {
          path: {
...
export function useArchiveCategory(
  ...
  const queryClient = useQueryClient()
  const mutationOptions = archiveCategoryMutation()
  return useMutation<ArchiveCategoryResponse, Error, ArchiveCategoryVariables>({
    mutationFn: async (variables: ArchiveCategoryVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing archiveCategory mutation function')
      return mutationFn(
        {
          path: {
            categoryId: variables.id
