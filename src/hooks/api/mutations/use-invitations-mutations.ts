...
import type { CreateInvitationResponse } from '@/api/generated/types.gen'
...
type AcceptInvitationVariables = InvitationVariables & {
  householdId?: string
}
...
export function useCreateInvitation(
  ...
  >({
    mutationFn: async (variables: CreateInvitationVariables) => {
      ...
      if (!mutationFn)
        throw new Error('Missing createInvitation mutation function')
      return mutationFn(
        {
          path: {
...
export function useAcceptInvitation(
  ...
  >({
    mutationFn: async (variables: AcceptInvitationVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing acceptInvitation mutation function')
      await mutationFn(
        {
          path: {
...
export function useDeclineInvitation(
  ...
  return useMutation<unknown, Error, InvitationVariables>({
    mutationFn: async (variables: InvitationVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing declineInvitation mutation function')
      return mutationFn(
        {
          path: {
            invitationId: variables.invitationId
...
export function useRevokeInvitation(
  callbacks?: MutationCallbacks<unknown, RevokeInvitationVariables>
) {
  const queryClient = useQueryClient()
  ...
  return useMutation<unknown, Error, RevokeInvitationVariables>({
    mutationFn: async (variables: RevokeInvitationVariables) => {
      const mutationFn = mutationOptions.mutationFn
      if (!mutationFn)
        throw new Error('Missing revokeInvitation mutation function')
      return mutationFn(
        {
          path: {
            invitationId: variables.invitationId
