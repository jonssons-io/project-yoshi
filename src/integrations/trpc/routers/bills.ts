...
import { addDays, addWeeks, addMonths, addYears, startOfDay, isWithinInterval } from 'date-fns'
...
export const billsRouter = {
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (thisMonthOnly && !includeArchived) {
        ...
        return billsWithStatus.filter((bill) => {
          ...
          return isWithinInterval(bill.nextOccurrence, { start: monthStart, end: monthEnd })
        })
      }
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.recurrenceType === RecurrenceType.CUSTOM && !input.customIntervalDays) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
