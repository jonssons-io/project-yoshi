  ...
  z.object({
    ...
    recipient: z.union([
      ...
      z.object({
        ...
        name: z.string().min(1, {
          message: t('validation.recipientNameRequired')
        })
      })
    ]),
    ...
    >
      ...
      {budgets.length > 0 ? (
        <form.AppField
          name="budgetId"
          validators={{
            onChange: createZodValidator(billSchema.shape.budgetId)
          }}
        >
          {(field) => (
            <field.SelectField
              ...
              onCheckedChange={async (checked) => {
                if (!checked && form.getFieldValue('splits').length > 1) {
                  const isConfirmed = await confirm({
                    description: t('forms.disableSplitBill'),
      ...
      <form.AppField
        name="endDate"
        validators={{
          onChange: createZodValidator(billSchema.shape.endDate)
        }}
      >
        {(field) => (
          <field.DateField
