...
export function SplitExpenseBlock({
  form,
  householdId,
  userId,
  expandedSplitIds,
  ...
  return (
    <form.Subscribe selector={(s) => s.values.splits as SplitRowValue[]}>
      {(splits) => {
        const total = splits.reduce((s, r) => s + (Number(r.amount) || 0), 0)
        return (
          <div className="flex flex-col gap-4">
            <SyncSplitTotalToAmount
              form={form}
              splits={splits}
            />
...
function CreateTransactionSplitSection({
  form,
  index,
  householdId,
  userId,
      ...
      {({ subtitle, amount, budgetId }) => {
        ...
        return (
          <div className="flex w-full flex-col gap-2">
            ...
            {expanded ? (
              <CreateTransactionSplitSectionFields
                form={form}
                index={index}
                householdId={householdId}
                userId={userId}
...
function CreateTransactionSplitSectionFields({
  form,
  index,
  householdId,
  userId,
