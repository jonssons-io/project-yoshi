...
function buildAccountBalance(account: (typeof accounts)[number]) {
  return {
    account: {
      id: account.id,
      name: account.name
    ...
    transactionCount: transactions.filter((txn) => txn.accountId === account.id)
      .length,
    staleHistory: false,
    historyRecalcFrom: null
  }
}

export const accountHandlers = [
  http.get(
    `${BASE}/households/:householdId/accounts`,
    ({ request, params }) => {
  ...
  http.patch(`${BASE}/accounts/:accountId/archive`, ({ params }) => {
    ...
    accounts[index] = {
      ...accounts[index],
      archived: !accounts[index].archived
    }
    return HttpResponse.json(accounts[index])
  }),
  ...
  http.get(`${BASE}/accounts/:accountId/balance`, ({ params }) => {
    ...
    return HttpResponse.json(buildAccountBalance(account))
  }),

  http.get(
    `${BASE}/households/:householdId/account-balances`,
    ({ request, params }) => {
      const url = new URL(request.url)
      const includeArchived = url.searchParams.get('includeArchived') === 'true'
      const requestedAccountIds = url.searchParams.getAll('accountIds')
      const filtered = accounts.filter((account) => {
        if (account.householdId !== params.householdId) return false
      ...
      return HttpResponse.json({
        data: filtered.map(buildAccountBalance)
      })
    }
  ),
  ...
  http.get(`${BASE}/accounts/:accountId/balance-history`, ({ params }) => {
    ...
    return HttpResponse.json({
      ...
      history: [
        ...
        {
          date: '2026-01-15',
          balance: account.currentBalance - 200
        },
        {
          date: '2026-01-31',
